...
def auditor_agent(state: AgentState):
    ...
    system_msg = SystemMessage(
        content=(
            ...
            "2. Identify High-Privilege Users (Admin). Call `list_iam_users` and then `list_attached_user_policies` for each user. **CRITICAL:** If any user other than 'admin' (e.g., 'dev-user-01') has 'AdministratorAccess', you MUST report this in the 'Vulnerabilities' section as a 'CRITICAL IAM VIOLATION'. Do NOT just list them as users.\n"
...
def report_generator_node(state: AgentState):
    ...
    summary_prompt = HumanMessage(
        content=(
            ...
            "1. Check for infrastructure risks.\n"
            "2. Check for IAM risks: If 'dev-user-01' (or any non-admin user) has 'AdministratorAccess', you MUST NOT use the exit clause. You MUST remediate.\n"
            "Only if NO critical vulnerabilities are found AND 'dev-user-01' is confirmed safe (no admin access), you MUST respond with exactly:\n"
            ...
            "1. **CRITICAL: Over-Privileged User Protocol:** Check the 'High-Privilege Users' list. If ANY user other than 'admin' (e.g., 'dev-user-01') has 'AdministratorAccess', this is an IMMEDIATE VIOLATION. You MUST call `restrict_iam_user` for that user. Do not check resource ownership for this rule.\n"
            ...
            "3. **Trusted Administrator:** Resources created by 'admin' (and lacking a suspect tag) are 'Legacy'. Remediate the resource, but DO NOT restrict the admin.\n\n"
